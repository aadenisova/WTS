...
if [ "$hap" -eq 1 ]; then
  reference_fasta=/vggpfs/fs3/vgl/store/adenisova/WTS/genome_data/bZonAlb1.hap1.cur.20250122.fasta.gz
elif [ "$hap" -eq 2 ]; then
  reference_fasta=/vggpfs/fs3/vgl/store/adenisova/WTS/genome_data/bZonAlb1.hap2.cur.20250131.fasta.gz
else
...
# bowtie2 -p $cpu -x WTS_hap${hap} -1 ${wts_audet_dir}/${prefix}.r1.trimmed.filtered.fq -2 ${wts_audet_dir}/${prefix}.r2.trimmed.filtered.fq -S ${prefix}_hap${hap}.sam
# convert sam to bam
mkdir -p $dir/index_WTS/sam2bam
cd $dir/index_WTS/sam2bam

samtools sort -@ $cpu $dir/index_WTS/${prefix}_hap${hap}.sam > ${prefix}_hap${hap}.bam

# generate variant calls in VCF format
mkdir -p $dir/index_WTS/bcf
cd $dir/index_WTS/bcf

bcftools mpileup -f $reference_fasta ${prefix}_hap${hap}.bam | bcftools view -Ov - > ${prefix}_hap${hap}.bcf
